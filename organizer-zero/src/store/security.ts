...
class SecurityManager {
  ...
  private setupAutoLock() {
    const timeout = settingsManager.getSetting('autoLockTimeout');
    if (timeout && timeout > 0) {
      setInterval(() => {
        const now = Date.now();
        const elapsed = (now - this.state.lastActive) / 60000; // minutes
  ...
  private getSecret(): string {
    const requireAuth = settingsManager.getSetting('requireAuth');
...
export type SecurityManagerAPI = typeof securityManager;